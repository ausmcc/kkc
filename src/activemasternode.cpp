...
//
// Bootup the masternode, look for a 10000 KKC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in koalakash.conf */
...
{
    CkoalakashcoinAddress address(collateralAddress);
