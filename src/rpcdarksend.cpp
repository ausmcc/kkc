...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <koalakashaddress> <amount>\n"
            "koalakashaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <koalakashaddress> <amount>\n"
            "koalakashaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in koalakash.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in koalakash.conf\n"
                ...
                "  vote-many    - Vote on a Koalakash initiative\n"
                "  vote         - Vote on a Koalakash initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CkoalakashcoinSecret(secret).ToString();
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Koalakash initiative (can be additionally filtered, partial match)\n"
