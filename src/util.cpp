...
int nAnonymizekoalakashAmount = 1000;
...
#else
    const char* pszModule = "koalakash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Koalakash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Koalakash
    // Mac: ~/Library/Application Support/Koalakash
    // Unix: ~/.koalakash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Koalakash";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "koalakash.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "koalakashd.pid"));
