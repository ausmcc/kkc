...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizekoalakashAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizekoalakashAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizekoalakashAmount = strAnonymizekoalakashAmount.remove(strAnonymizekoalakashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizekoalakashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizekoalakashAmount*COIN) nMaxToAnonymize = nAnonymizekoalakashAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizekoalakashAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizekoalakashAmount));
        strAnonymizekoalakashAmount = strAnonymizekoalakashAmount.remove(strAnonymizekoalakashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizekoalakashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizekoalakashAmount == 0){
