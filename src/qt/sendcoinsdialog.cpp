    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Koalakash address (e.g. Koalakash_Address_Here_QsAqFckyf9pRHN)"));
#endif
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        CkoalakashcoinAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::IXTransactionCreationFailed:
        msgParams.first = tr("InstantX doesn't support sending values that high yet. Transactions are currently limited to 1000 KKC.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CkoalakashcoinAddress addr = CkoalakashcoinAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Koalakash address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
