...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("koalakash"))
        return false;
...
{
    // Convert koalakash:// to koalakash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("koalakash://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "koalakash:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Koalakash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Koalakash.lnk
...
{
    return GetAutostartDir() / "koalakash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a koalakash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Koalakash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the koalakash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add koalakash app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Koalakash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  koalakash-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Koalakash-Qt"));
