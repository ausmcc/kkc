...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    else
    {
        ...
        {
            // Credit
            if (CkoalakashcoinAddress(rec->address).IsValid())
            {
                CTxDestination address = CkoalakashcoinAddress(rec->address).Get();
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            {
                ...
                if (!wtx.mapValue.count("to") || wtx.mapValue["to"].empty())
                {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address))
                    {
                        ...
                        strHTML += GUIUtil::HtmlEscape(CkoalakashcoinAddress(address).ToString());
            ...
            if(txdb.ReadDiskTx(prevout.hash, prev))
            {
                if (prevout.n < prev.vout.size())
                {
                    ...
                    if (ExtractDestination(vout.scriptPubKey, address))
                    {
                        if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].empty())
                            strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address]) + " ";
                        strHTML += QString::fromStdString(CkoalakashcoinAddress(address).ToString());
                    }
