...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizekoalakashAmount"))
        settings.setValue("nAnonymizekoalakashAmount", 1000);
    nAnonymizekoalakashAmount = settings.value("nAnonymizekoalakashAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizekoalakashAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizekoalakashAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizekoalakashAmount:
            return QVariant(nAnonymizekoalakashAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizekoalakashAmount:
            nAnonymizekoalakashAmount = value.toInt();
            settings.setValue("nAnonymizekoalakashAmount", nAnonymizekoalakashAmount);
            emit AnonymizekoalakashAmountChanged(nAnonymizekoalakashAmount);
            break;
        default:
            break;
        }
