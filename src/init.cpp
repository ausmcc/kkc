...
void Shutdown()
{
    ...
    RenameThread("koalakash-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: koalakash.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: koalakashd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizekoalakashamount=<n> " + _("Keep N Koalakash anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Koalakash is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Koalakash is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Koalakash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Koalakash server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Koalakash") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Koalakash to complete") << "\n";
    ...
    nAnonymizekoalakashAmount = GetArg("-anonymizekoalakashamount", 0);
    if(nAnonymizekoalakashAmount > 999999) nAnonymizekoalakashAmount = 999999;
    if(nAnonymizekoalakashAmount < 2) nAnonymizekoalakashAmount = 2;
    ...
    LogPrintf("Anonymize Koalakash Amount %d\n", nAnonymizekoalakashAmount);
