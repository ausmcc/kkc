...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out, bool fIncludeHex)
{
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(CkoalakashcoinAddress(addr).ToString());
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        {
            CkoalakashcoinAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Koalakash address: ")+input.get_str());
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address))
        {
            entry.push_back(Pair("address", CkoalakashcoinAddress(address).ToString()));
...
Value decodescript(const Array& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", CkoalakashcoinAddress(script.GetID()).ToString()));
    return r;
}
