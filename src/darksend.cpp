    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Koalakash and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002KKC per transaction on average.
            */
            if(r <= 10)
            {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizekoalakashAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100KKC+1 ( bit on if present )
    // bit 1 - 10KKC+1
    // bit 2 - 1KKC+1
    // bit 3 - .1KKC+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("koalakash-darksend");
